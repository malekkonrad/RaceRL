...
//     public float motorTorque = 2000f; // Moc silnika
//     public float brakeTorque = 4000f; // Moc hamulców
//     public float maxSpeed = 60f;      // Maksymalna prędkość w m/s (60 m/s ~ 216 km/h)
    
//     [Header("Steering")]
//     public float maxSteerAngle = 30f;
//     public float steerSpeed = 10f; // Jak szybko koła skręcają

//     [Header("Arcade Assists")]
//     public float downforce = 50f;     // Docisk (zapobiega lataniu)
//     public bool keepUpright = true;   // Czy wymuszać pionową pozycję (anty-wywrotka)
...
//     {
//         rb = GetComponent<Rigidbody>();
//         // Obniżamy środek ciężkości - KLUCZOWE dla stabilności
//         rb.centerOfMass = new Vector3(0, -0.5f, 0.1f);
//         rb.interpolation = RigidbodyInterpolation.Interpolate; // Płynniejszy ruch kamery
//     }

//     public void SetInputs(float forwardAmount, float turnAmount)
//     {
//         // 1. STEROWANIE KOŁAMI
//         float targetAngle = turnAmount * maxSteerAngle;
//         currentSteerAngle = Mathf.Lerp(currentSteerAngle, targetAngle, Time.fixedDeltaTime * steerSpeed);

//         if(frontLeft) frontLeft.transform.localRotation = Quaternion.Euler(0, currentSteerAngle, 0);
//         if(frontRight) frontRight.transform.localRotation = Quaternion.Euler(0, currentSteerAngle, 0);

//         // 2. NAPĘD I HAMOWANIE
//         // Aplikujemy siłę tylko jeśli koła dotykają ziemi
//         if (rearLeft.IsGrounded() && rearRight.IsGrounded())
//         {
//             float currentSpeed = Vector3.Dot(transform.forward, rb.linearVelocity);
            
//             // Przyspieszanie (tylko jeśli nie przekroczyliśmy maxSpeed)
//             if (forwardAmount > 0.1f && currentSpeed < maxSpeed)
//             {
//                 rb.AddForce(transform.forward * forwardAmount * motorTorque);
//             }
//             // Cofanie
//             else if (forwardAmount < -0.1f && currentSpeed > -20f)
//             {
//                 rb.AddForce(transform.forward * forwardAmount * motorTorque);
//             }
//         }
...
//         // 3. HAMOWANIE ARCADE
//         // Jeśli wciskamy hamulec (np. input < 0 jadąc do przodu), aplikujemy siłę przeciwną do ruchu
//         // To jest stabilniejsze niż wheel collidery
//         bool isBraking = (forwardAmount < -0.1f && Vector3.Dot(transform.forward, rb.linearVelocity) > 1f) ||
//                          (forwardAmount > 0.1f && Vector3.Dot(transform.forward, rb.linearVelocity) < -1f);

//         if (isBraking)
//         {
//             rb.AddForce(-rb.linearVelocity.normalized * brakeTorque);
//         }
//     }

//     void FixedUpdate()
//     {
//         // 4. DOCISK AERODYNAMICZNY
//         // Im szybciej, tym mocniej dociskamy, ale bez przesady
//         float speedFactor = rb.linearVelocity.magnitude / maxSpeed;
//         rb.AddForce(-Vector3.up * downforce * rb.mass * speedFactor);

//         // 5. SZTUCZNA STABILIZACJA (Keep Upright)
//         // To zapobiega wywracaniu się i przechylaniu na zakrętach
//         if (keepUpright)
//         {
//             // Przewidujemy rotację, która wyrówna auto do poziomu
//             Quaternion targetRot = Quaternion.FromToRotation(transform.up, Vector3.up) * transform.rotation;
//             // Delikatnie obracamy auto w stronę "pionu"
//             float stabilitySpeed = 5f;
//             transform.rotation = Quaternion.Lerp(transform.rotation, targetRot, Time.fixedDeltaTime * stabilitySpeed);
            
//             // Opcjonalnie: Zerujemy prędkość kątową na osi Z (roll), żeby nie bujało
//             var angularVel = transform.InverseTransformDirection(rb.angularVelocity);
//             angularVel.z *= 0.1f; // Tłumienie przechyłów bocznych
//             rb.angularVelocity = transform.TransformDirection(angularVel);
//         }
//     }
